...
<head>
  ...
  function init() {
	  ...
	  // load up active member list from DB 
    ...
    if ($likeDate != "0000-00-00" && $preloadOption == "L" ) {
        ...
        $con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	        ...
	        $result = $con->prepare("DELETE FROM assignment
			   WHERE date= '$schedDate' AND leagueID = '$leagueID' ");
		...
		try {                                               	
        $result = $con->query("SELECT * FROM assignment WHERE date = '$likeDate' AND leagueID = '$leagueID' ");
...
//                                                                	 load up available members into $available string
        ...
        $query = "SELECT   available.mbrID as 'mbrID',
                           member.assignedRanking		
		          FROM     available INNER JOIN member
				  ON       available.mbrID = member.mbrID AND available.leagueID = member.leagueID
				  WHERE    available.dates = '$schedDate' AND available.leagueID = '$leagueID'
	  ...
	  $j = -1;
		...
		for ($i=0; $i <= sizeof($availableArray); $i++) {
			...
			if ($player != "" && $schedDate != "0000-00-00" ){	
				...
				$result = $con->prepare("DELETE FROM assignment
					WHERE mbrID = '$player' AND date= '$schedDate' AND leagueID = '$leagueID' ");
...
//                                                                       process input fields
  if (filter_has_var(INPUT_POST, "player")) {
	...
	for ($i=0; $i < sizeof($players); $i++) {
      ...
      if ($player != "*None" && $court != "" && $schedDate != "0000-00-00" && $action == "A"){		
        try {
		...
		if ($ballSuppliers[0] != "") {
			...
			try {			        
				...
				for ($i=0; $i < sizeof($ballSuppliers); $i++) { 
					...
					$result = $con->prepare("UPDATE assignment
		                              SET balls = 'Y'
					                  WHERE date = '$schedDate' AND mbrID = '$ballSuppliers[$i]' AND leagueID = '$leagueID' ");
...
print <<<HERE
    <form action = "" method= "post">
      <fieldset>
		<br><label>Player(s)</label>
        <select id="memberList"  name="player[]" multiple size=9 >
		</select>
    ...
    $assigned = "";	                                                    //       load up assigned members into $assigned string
      try {
        ...
        $query = "SELECT facilityName as 'Courts  ', 
		                 mbrID as 'Players' FROM assignment WHERE date = '$schedDate' AND leagueID = '$leagueID'
	    ...
	    $query = "SELECT mbrID, balls FROM assignment WHERE date = '$schedDate' AND leagueID = '$leagueID' ";
...
<?php
	  ...
	  $i = 0;  $k =0;                                                 // display unassigned, but available
