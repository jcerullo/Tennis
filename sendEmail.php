...
<?php
	...
	$ballSuppliers = "";                                         //  load up ball suppliers
		
	   try {
        $con= new PDO('mysql:host=localhost;dbname=tennis', "root", "jjc003"); 
        $con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		                                                               
        $query = "SELECT emailAddr 
		  FROM member 
		  WHERE status = 'A' AND leagueID = '$leagueID' ";      //start load of active members email addresses
        ...
        $query = "SELECT emailAddr 
		  FROM member 
		  WHERE status = 'I' AND leagueID = '$leagueID' ";              //start load of inactive members email addresses
		...
		$date = $dateArray[0];										//  start load of ball suppliers
		
		$query = "SELECT emailAddr FROM assignment 
		                           INNER JOIN member
								   ON assignment.mbrID = member.mbrID
								   WHERE assignment.date = '$date' AND assignment.balls = 'Y'
								   ";                     
        ...
        $ballSuppliers .= "";                                              //  end of load up of ball suppliers
	...
	if ($body != "B" && $subj == "R") {
		$body = "<p> Please provide your $monthNames[$month] availability to play <br>
                ...
                <p>You can help by entering your availability dates at <br> 
                   https://thevillages.duckdns.org/tennis</p>
	...
	elseif ($to == "M" && $subj == "W") 
	{
		$body = "<p>Sir -</p>
                ...
                <p>Welcome to $leagueName.  After reading this email, please click on the link below
				to enter the information that we require for communication purposes.<br></p>
				
                 <p>  https://thevillages.duckdns.org/tennis </p>
...
Your temporary password is: password. Your member ID is your first name and last name initial, as indicated above.
...
The menu list at the top of every screen takes you anywhere you want to go in the system. 
...
If you encounter any problems, please email jcerullo@yahoo.com
	...
	elseif ($body != "B" && $subj == "N") 
	{
		$body = "<p>Gentlemen -</p>
                ...
                <p>START TIME: <strong>8:00</strong>AM</p>

                <p>PLEASE call $moderatedBy if you need <br>
				...
				<img src='https://thevillages.duckdns.org/tennis/logo.jpg' alt= 'League Logo'/>
                ";                       
	}
	                                                         // notification to supply balls
	elseif ($body != "B" && $subj == "S") 
	{
		$body = "<p>Gentlemen -</p>

                <p><strong>PLEASE be aware that you are to bring one new can of balls to <br>
				   your assigned court on $date at 8:00 AM.</strong></p>
    ...
    elseif ($subj == "W")
	{
		$subj = "Welcome to the $leagueName";
	}		
	elseif ($subj == "P")
	{ 
	...
	elseif ($subj == "S")
	{ 
		$subj = "Notification to supply balls";
	}
      ...
      <fieldset> <br>
        ...
        <label>To:</label>
          <select id="selTo"  name="to" ><br>
			...
			<option value = "B" >Ball Suppliers</option>
		  </select>
		...
		<select id="selSubj"  name="subj" ><br>
			...
			<option value = "S" >Notification to supply balls</option>
		</select>
