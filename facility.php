...
<?php
    ...
    if (filter_has_var(INPUT_POST, "facilityName")){
		...
		$contractDays = filter_input(INPUT_POST, "contractDays");
		                                                        // load $facilities array
	    if ($facilityName == "*all") {
		...
		try {       
				...
				$con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                                                                   ...
                                                                   // update data base for all facilities
		...
		for ($i = 0; $i < sizeof($facilities); $i++) {	
			...
			if ($facilityNbr != 0) {
				$result = $con->prepare("UPDATE facility 
			          ...
			          WHERE facilityName = '$facilityName' AND leagueID = '$leagueID' ");
				$result->execute();
			}
			
			if ($nbrOfCourts != 0 && $nbrOfCourts <= 9) {
				$result = $con->prepare("UPDATE facility 
...
<nav id="nav">
  ...
  <style type = "text/css">
    ...
    <form action = "facility.php" method= "post">
      <fieldset> <br>	  
		...
		<label>Change Days to</label>
        <input type="text" value="" id="txt_contractDays" name="contractDays" value = "contractDays"> <br>
  ...
  <style type = "text/css">
	...
	#memberList {
	  ...
	  <h2><strong> Facility availability detail </strong></h2>  
...
<?php
      try {
        ...
        $query = "SELECT *
		                 FROM facility
                         INNER JOIN league
						 ON facility.leagueID = league.leagueID							 
						 ORDER BY facility.leagueID, facilityNbr ";
        ...
        print "  <tr> \n";
        foreach ($row as $field => $value){
		  if ($field == leagueName) print " <th> League  <span style='margin-left:1em'></th> \n";
		  if ($field == facilityName) print " <th> Facility Name  <span style='margin-left:1em'></th> \n"; 
		  if ($field == nbrOfCourts) print " <th> Courts <span style='margin-left:1em'></th> \n";
          if ($field == startDate) print " <th> Start Date <span style='margin-left:2em'></th> \n";
          if ($field == endDate) print " <th> End Date <span style='margin-left:2em'></th> \n"; 
          if ($field == startTime) print " <th> Start / <span style='margin-left:1em'></th> \n";
          if ($field == endTime) print " <th> End Time <span style='margin-left:1em'></th> \n"; 
          if ($field == contractDays) print " <th> Days <span style='margin-left:2em'></th> \n"; 		  
        } // end foreach
        print "  </tr> \n";
        ...
        foreach($data as $row){
          ...
          foreach ($row as $name=>$value){
			...
			if ($name == 'contractDays') {
                $contractDays = $value;
				print "    <td>$contractDays</td> \n";
			}
