<?php
  session_start();
  $leagueID = $_SESSION["leagueID"];                   //fetch league info
	try {                                                
...
<?php
	...
	if (filter_has_var(INPUT_POST, "leagueID")){
...
//                                                          load facilities array to determine max facility nbr
	...
	try {
        ...
        $query = "SELECT facilityName, facilityNbr 
		           FROM facility
	  ...
	  if ($facility != "" && $leagueID != "") {
        ...
        $con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			...
			$result = $con->prepare("DELETE FROM facility 
			   WHERE facilityName = '$facility' AND leagueID = '$leagueID'");
			   
			$result->execute();
                                                           // then add SQL record
			 
	        $result = $con->prepare("INSERT INTO facility 
			            (facilityName, facilityNbr, leagueID, contractDays)			
			     VALUES
			            ('$facility', '$newFacilityNbr', '$leagueID', '$contractDays') ");
  ...
  <style type = "text/css">
    ...
    <form action = "" method= "post">
      <fieldset> <br>	  
        ...
        <label>Days (e.g. MWF) </label>
        <input type="text" value="" id="txt_contractDays" name="contractDays" pattern="[A-Za-z/]{1,9}"> <br>		
...
<?php      
	  print " <h2><strong> The contracted facilities with $leagueName are: </strong></h2>";
	   
      try {
        $con= new PDO('mysql:host=localhost;dbname=tennis', "root", "jjc003");
        $con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        $query = "SELECT  facilityName as 'Facility Name', contractDays as 'Days' 		
		            FROM facility 
                    WHERE leagueID = '$leagueID'					
					ORDER BY leagueID ";
