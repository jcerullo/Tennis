    ...
    if (filter_has_var(INPUT_POST, "selMbr")){
        ...
        $con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	        ...
	        $result = $con->prepare("UPDATE member 
			          ...
			          WHERE mbrID = '$selMbr' AND leagueID = '$leagueID' ");
			}
...
?>
  ...
  <style type = "text/css">
	...
	#memberList {
	  ...
	  <h2><strong> Tennis member rankings are: </strong></h2>  
      <p>  
<?php
      try {
        $con= new PDO('mysql:host=localhost;dbname=tennis', "root", "jjc003");
        ...
        foreach ($row as $field => $value){
		  ...
		  if ($field == assignedRanking && $isAdmin) print " <th> Administrator's Ranking <span style='margin-left:6em'></th> \n"; 
        } // end foreach
        print "  </tr> \n";
        ...
        foreach($data as $row){
          ...
          foreach ($row as $name=>$value){              
			...
			if ($name == 'assignedRanking' && $isAdmin) {
