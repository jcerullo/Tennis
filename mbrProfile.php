...
<?php
  ...
  init();
	...
	try {                                                // retrieve SQL record
        ...
        $result = $con->query("SELECT * FROM member WHERE mbrID = '$mbrID' AND emailAddr != '' ");
		$result->setFetchMode(PDO::FETCH_ASSOC);
		...
		foreach ($result as $row) {
		  foreach ($row as $name =>$value ) {                               // populate form values for form display
			...
			if ($name == 'phoneSecondary' && $value != "")   $phoneSecondary  = $value;
			if ($name == 'village' && $value != "")   $village  = $value;
			if ($name == 'streetAddr' && $value != "")   $streetAddr  = $value;
			if ($name == 'cityAddr' && $value != "")   $cityAddr  = $value;
			if ($name == 'zipAddr' && $value != "")   $zipAddr  = $value;
			if ($name == 'emailAddr' && $value != "")   $emailAddr  = $value;
			if ($name == 'emailAddr' && $value != "")   $_SESSION["myEmailAddr"] = $value;   // set my email address
			if ($name == 'status' && $value != "")   $status  = $value;
			if ($name == 'password' && $value != "")   $password  = $value;
			if ($name == 'tennisRanking' && $value != "")   $tennisRanking  = $value;
			if ($name == 'displayPhoto' && $value != "")   $displayPhoto  = $value;
		  }
...
<?php
print "<span style='margin-left:1em'> You are logged in as <strong> $mbrID </strong> on the <strong>Member Profile </strong> page of $leagueName. ";
